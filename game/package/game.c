...
int guess_enter_number(void)
{
	...
	while (loop) {
        ...
        if ((guess_enter/1000) == 0 || (guess_enter/1000) >= 10) {
            ...
            loop = contrast_number(loop, dismantle_create_number
                                    , dismantle_guess_number);
            guess_time->guess_score = count+1;
        }
	}

	file_value = open("./lib/guess_list.txt", O_WRONLY, 0777);
    if (file_value == CRAZY_FILE_ERROR) {
        file_value = open("./lib/crazy_list.txt", O_WRONLY, 0777);
        if (file_value == CRAZY_FILE_ERROR) {
            printf("crazy_file create error");
               exit(0);
        }   
    }
    lseek(file_value, 0, SEEK_END);
	return_value = write(file_value, guess_time, sizeof(struct game_list_score));
 ...
 *          
 */
int contrast_number(int loop, int create_number[4], int guess_number[4])
{
    int run;
	int count;
	int count_a = 0;
...
{   
    ...
    lseek(file_value, 0, SEEK_END);
	return_value = write(file_value, crazy_number, sizeof(struct game_list_score));
    if (return_value == CRAZY_FILE_ERROR) {
        DEBUG("write crazy_list.txt error");
    }
	close(file_value);
